...
/**
 * Forces Cache-Control: no-store on index.html in dev mode,
 * and rewrites the main entry <script> src to include a timestamp
 * query param so browsers can never serve a stale cached module.
 */
	...
	return {
		...
		configureServer(server) {
			server.middlewares.use((req, res, next) => {
				...
				// Strip any ?t= cache-buster from module requests so Vite
				// can resolve them normally (Vite uses its own ?v= versioning).
				if (req.url?.startsWith('/src/main.tsx?')) {
		...
		transformIndexHtml(html) {
			// Stamp the entry script with a build-time timestamp so the browser
			// never serves a stale cached version of main.tsx.
			const ts = Date.now();
			return html.replace(
				/src="\/src\/main\.tsx"/,
				`src="/src/main.tsx?t=${ts}"`
			);
		},
	};
}
...
export default defineConfig(({ mode }) => ({
	...
	optimizeDeps: {
		...
		// Force all packages to share the same pre-bundled React instance
		esbuildOptions: {
			define: {
				global: "globalThis",
			},
	...
	resolve: {
		// Hard-deduplicate React across every package in the graph.
		// This is the definitive fix for "Cannot read properties of null (reading 'useState')".
		// Any package (including Firebase) that imports React will always get
		// the same singleton that was pre-bundled in the optimizeDeps pass.
		dedupe: [
			"react",
			"react-dom",
			"react/jsx-runtime",
